...
This function triggers the `gfa_data_silver` in mage. This cloud function is event driven; it runs every time a new file is placed into the `gfa_data` bucket in Google Cloud Storage. This function contains logic to fire of the mage pipeline only if it's placed in the `raw` subfolder of the `gfa_data` bucket. 

It  trigger the mage pipeline via a post request and passes these runtime variables from the [gfa pipeline](https://github.com/CarrierOps/1P-Wiki/blob/main/DocuMentor/pipelines/gfa.md):
...
Make sure you're added as a contributor to [the github repo](https://github.com/CarrierOps/gcp-ppln-trigger-gfa-silver.git). (If you click on the link and you can see things, then you're good to go, if not, contact Thomas or Michael to be added as a contributor).
...
<figure align="center">
    <img src="../../../imgs/trigger_gfa_high_level.png" width="95%">
  <figcaption>High-level diagram showing the how trigger_gfa_silver_ppln Cloud Function DevOps works.</figcaption>
</figure>
