...
## Summary
This function triggers the `att_sales_silver` pipeline in mage. This cloud function is event driven; it runs every time a new file is placed into the `sell-through` bucket in Google Cloud Storage. This function contains logic to fire off the mage pipeline only if it's placed in the `raw` folder of the  `att_daily_sales` subfolder within the `sell-through` bucket. 

It  trigger the mage pipeline via a post request and passes these runtime variables from the [att pipeline](https://github.com/CarrierOps/1P-Wiki/blob/main/DocuMentor/pipelines/att.md)
...
Make sure you're added as a contributor to [the github repo](https://github.com/CarrierOps/gcp-ppln-trigger-att-sales-silver.git). (If you click on the link and you can see things, then you're good to go, if not, contact Thomas or Michael to be added as a contributor)
...
> :bulb: **Quick-Tip**
>
> - After cloning the github repo, cd into it. From the command line, type `cd gcp-ppln-trigger-att` then hit <kbd>tab</kbd> to auto-complete then hit <kbd>enter</kbd>
>
...
<figure align="center">
    <img src="../../../imgs/trigger_att_high_level.png" width="95%">
